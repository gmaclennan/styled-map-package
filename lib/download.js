...
export function download({ bbox, maxzoom, styleUrl, onprogress, accessToken }) {
  ...
  ;(async () => {
    ...
    try {
      ...
      // @ts-ignore - ReadableStream.from() is available in Node 18+ but not in TS lib types
      await ReadableStream.from(tiles).pipeTo(writer.createTileWriteStream({ concurrency: 24 }))
      ...
      // @ts-ignore - ReadableStream.from() is available in Node 18+ but not in TS lib types
      await ReadableStream.from(glyphs).pipeTo(writer.createGlyphWriteStream())
