 ...
 * @param {string} mbtilesPath
 * @param {string} [outputPath]
 * @returns {Promise<void> | ReadableStream<Uint8Array>}
 */
export function fromMBTiles(mbtilesPath, outputPath) {
  ...
  const returnValue = outputPath
    ? writer.outputStream.pipeTo(Writable.toWeb(fs.createWriteStream(outputPath)))
    : writer.outputStream
