...
export class Reader {
  ...
  #closePromise
  /** @type {FileSource | null} */
  #fileSource = null

  /**
   * @param {string | Buffer | Uint8Array} filepathOrBuffer Path to styled map package (`.styledmap`) file, or a Buffer/Uint8Array of zip data
   */
  constructor(filepathOrBuffer) {
    /** @type {Promise<import('@gmaclennan/zip-reader').ZipReader>} */
    let zipPromise
    if (typeof filepathOrBuffer === 'string') {
      const sourcePromise = FileSource.open(filepathOrBuffer)
      ...
      const source = new BufferSource(filepathOrBuffer)
      zipPromise = ZipReader.from(source)
    }
    this.#zipPromise = zipPromise
    ...
    this.#entriesPromise = (async () => {
      ...
      try {
        ...
        // Close the file source on open failure to avoid FD leaks
        if (this.#fileSource) {
  ...
  async getVersion() {
    ...
    return (await text(versionEntry.readable())).trim()
  }
  ...
  async getStyle(baseUrl = null) {
    ...
    const style = await json(styleEntry.readable())
