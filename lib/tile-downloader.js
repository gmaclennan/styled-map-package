  ...
  const tiles = (async function* () {
    ...
    for (const [result, tileInfo] of queue) {
      ...
      if (mimeType) {
        ...
        // getTileFormatFromStream currently expects a Node Readable - convert
        const { Readable } = await import('node:stream')
        const nodeStream = Readable.fromWeb(body)
        let nodeStreamOut
        ;[format, nodeStreamOut] = await getTileFormatFromStream(nodeStream)
        body = Readable.toWeb(nodeStreamOut)
      }
      ...
      if (format === 'mvt') {
        ...
        stream = body.pipeThrough(new CompressionStream('gzip'))
      }

      // Wrap stream to call onDownloadComplete when fully consumed
      ...
      stream = stream.pipeThrough(trackingTransform)

      // Convert to Node Readable for backward compatibility with Writer.addTile
      const { Readable } = await import('node:stream')
      yield [Readable.fromWeb(stream), { ...tileInfo, format }]
    }
  })()
