...
async function fetchStream({ url, onresponse, onerror, onprogress }) {
  try {
    ...
    // Track consumption to know when the download is complete (to release pLimit slot)
    /** @type {(value?: any) => void} */ let resolveConsumed = () => {}
    /** @type {(err: unknown) => void} */ let rejectConsumed = () => {}
    const consumedPromise = new Promise((res, rej) => {
      resolveConsumed = res
      rejectConsumed = rej
    })
    ...
    response.body.pipeTo(trackingTransform.writable).catch(rejectConsumed)

    let body = trackingTransform.readable
    if (onprogress) {
    ...
    // Wait for the body to be fully consumed before resolving (releases pLimit slot)
