 ...
 *
 * @param {import('stream').Readable} tileData Node.js Readable stream
 * @returns {Promise<[import("../writer.js").TileFormat, import('stream').Readable]>}
 */
export async function getTileFormatFromStream(tileData) {
  // Convert to web stream for tee() support
  const { Readable } = await import('node:stream')
  const webStream = Readable.toWeb(tileData)
  ...
  // Convert stream2 back to a Node Readable for backward compatibility
  return [format, Readable.fromWeb(stream2)]
}
