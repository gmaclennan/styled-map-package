...
/** @typedef {string | Buffer | Uint8Array | import('stream').Readable | ReadableStream } Source */
    ...
    if (!format) {
      const webStream =
        typeof tileData === 'string'
          ? Readable.toWeb(fs.createReadStream(tileData))
          : toWebStream(tileData)
      // @ts-ignore - node:stream/web.ReadableStream is incompatible with global ReadableStream type
      ;[format, tileData] = await getTileFormatFromStream(webStream)
    }
...
function toWebStream(source) {
  if (typeof source === 'string') {
    // @ts-ignore - Readable.toWeb() returns node:stream/web.ReadableStream which is incompatible with global ReadableStream type
    return Readable.toWeb(Readable.from([Buffer.from(source)]))
  }
  if (Buffer.isBuffer(source) || source instanceof Uint8Array) {
    // @ts-ignore - Readable.toWeb() returns node:stream/web.ReadableStream which is incompatible with global ReadableStream type
    return Readable.toWeb(Readable.from([source]))
  }
  if (source && typeof /** @type {any} */ (source).pipe === 'function') {
    // Node.js Readable stream
    // @ts-ignore - Readable.toWeb() returns node:stream/web.ReadableStream which is incompatible with global ReadableStream type
    return Readable.toWeb(/** @type {import('stream').Readable} */ (source))
  }
  // Web ReadableStream
