...
export class Writer extends EventEmitter {
  ...
  constructor(style, { highWaterMark = 1024 * 1024 } = {}) {
    ...
    this.#outputStream = new Readable({ read() {} })
    const reader = this.#zipWriter.readable.getReader()
    ;(async () => {
      try {
        while (true) {
      ...
      const tileDataStream =
        typeof tileData === 'string'
          ...
          : tileData instanceof Uint8Array
            ...
            : tileData
      ;[format, tileData] = await getTileFormatFromStream(
        /** @type {import('stream').Readable} */ (tileDataStream),
      )
    }
