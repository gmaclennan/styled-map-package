...
describe('createTileWriteStream', () => {
  ...
  test('createTileWriteStream respects concurrency option', async () => {
    const style = {
      version: 8,
      sources: { test: { type: 'vector' } },
      layers: [{ id: 'bg', type: 'background' }],
    ...
    // Writing with concurrency=1 should work (serial)
    const ws = writer.createTileWriteStream({ concurrency: 1 })
    const wsWriter = ws.getWriter()
    await wsWriter.write([
      randomWebStream({ size: 256 }),
      {
        z: 0,
        x: 0,
        y: 0,
