...
test('tile-downloader: mid-stream errors do not prevent other tiles being yielded', async () => {
  ...
  const mockFetchQueue = /** @type {any} */ ({
    ...
    fetch: () => {
      ...
      return Promise.resolve({ body, mimeType: 'image/png', contentLength: null })
    },
  })
  ...
  assert.equal(yielded.length, 5, 'all tiles should be yielded even if streams error')
  assert.equal(tiles.skipped.length, 4, '4 tiles with stream errors should be skipped')
})
